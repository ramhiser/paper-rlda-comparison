...
gravier.error.rates <- function(k = 5, variable.selection = TRUE, q = 30, verbose = FALSE, seed) {
	set.seed(seed)
	n <- nrow(gravier)
	hold.out <- sample(seq_len(n), k)
	
	train.df <- gravier[-hold.out,]
	test.df <- gravier[hold.out,]
...
gravier.error.rates <- ddply(sim.configurations, .(k, q), function(sim.config) {
	cat("Leaving Out:", sim.config$k, "\tq:", sim.config$q, "\n")
	error.rates <- laply(seq_len(num.iterations), function(i) {
		gravier.error.rates(k = sim.config$k, variable.selection = TRUE, q = sim.config$q, verbose = verbose, seed = i)
...
save(gravier.error.rates, file = "rlda-gravier-sim-results.RData")