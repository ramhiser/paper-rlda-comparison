...
friedman.sim <- function(n.k, test.size, p, friedman.config, training.seed, test.seed, verbose = FALSE) {
	if(verbose) cat("Generating training data\n")
	train.df <- friedman.data(n1 = n.k, n2 = n.k, n3 = n.k, p = p, experiment = friedman.config, seed = training.seed)
	...
	test.df <- friedman.data(n1 = test.size, n2 = test.size, n3 = test.size, p = p, experiment = friedman.config, seed = test.seed)
...
sim.configurations <- expand.grid(sample.sizes, dim.features, friedman.config)
names(sim.configurations) <- c("n.k", "p", "friedman.config")

sim.error.rates <- adply(sim.configurations, 1, function(sim.config) {
	cat("n.k:", sim.config$n.k, "\tp:", sim.config$p, "\tFriedman Config:", sim.config$friedman.config, "\n")
	error.rates <- laply(seq_len(num.replications), function(i) {
		friedman.sim(n.k = sim.config$n.k,
			test.size = test.size,
			p = sim.config$p,
			friedman.config = sim.config$friedman.config,
			training.seed = i,
			test.seed = 1000 + i)
...
save(sim.error.rates, file = "rlda-friedman-sim-results.RData")