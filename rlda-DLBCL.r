...
DLBCL.error.rates <- function(k = 5, variable.selection = TRUE, q = 30, verbose = FALSE, seed) {
	set.seed(seed)
	n <- nrow(DLBCL.df)
	hold.out <- sample(seq_len(n), k)
	
	train.df <- DLBCL.df[-hold.out,]
	test.df <- DLBCL.df[hold.out,]
...
if(run.locally) {
	...
	DLBCL.df <- read.csv(bzfile("~/RameyPackagesR/data-microarray/data/DLBCL-shipp/DLBCL.csv.bz2", "r"))
}
sim.configurations <- expand.grid(hold.out.sizes, q)
names(sim.configurations) <- c("k", "q")

DLBCL.error.rates <- ddply(sim.configurations, .(k, q), function(sim.config) {
	cat("Leaving Out:", sim.config$k, "\tq:", sim.config$q, "\n")
	error.rates <- laply(seq_len(num.iterations), function(i) {
		DLBCL.error.rates(k = sim.config$k, variable.selection = TRUE, q = sim.config$q, verbose = verbose, seed = i)
...
save(DLBCL.error.rates, file = "rlda-DLBCL-sim-results.RData")