...
christensen.error.rates <- function(k = 5, variable.selection = TRUE, q = 30, verbose = FALSE, seed) {
	set.seed(seed)
	n <- nrow(christensen)
	hold.out <- sample(seq_len(n), k)
	
	train.df <- christensen[-hold.out,]
	test.df <- christensen[hold.out,]
...
sim.configurations <- expand.grid(k = hold.out.sizes, q = q)

christensen.error.rates <- ddply(sim.configurations, .(k, q), function(sim.config) {
	cat("Leaving Out:", sim.config$k, "\tq:", sim.config$q, "\n")
	error.rates <- laply(seq_len(num.iterations), function(i) {
		christensen.error.rates(k = sim.config$k, variable.selection = TRUE, q = sim.config$q, verbose = verbose, seed = i)
	}, .parallel = parallel, .progress = "text")
	error.rates.df <- data.frame(error.rates)
	names(error.rates.df) <- c("mlda", "nlda", "lda-pseudo", "mdeb", "mdeb-pool", "rlda-grid")
	error.rates.df
})

save(christensen.error.rates, file = "rlda-christensen-sim-results.RData")