...
chiaretti.error.rates <- function(k = 5, variable.selection = TRUE, q = 30, verbose = FALSE, seed) {
	set.seed(seed)
	n <- nrow(chiaretti.ALL.df)
	hold.out <- sample(seq_len(n), k)
	
	train.df <- chiaretti.ALL.df[-hold.out,]
	test.df <- chiaretti.ALL.df[hold.out,]
...
if(run.locally) {
	...
	chiaretti.ALL.df <- read.csv(bzfile("~/RameyPackagesR/data-microarray/data/chiaretti-ALL/chiaretti-ALL.csv.bz2", "r"))
}
sim.configurations <- expand.grid(hold.out.sizes, q)
names(sim.configurations) <- c("k", "q")

chiaretti.error.rates <- ddply(sim.configurations, .(k, q), function(sim.config) {
...
save(chiaretti.error.rates, file = "rlda-chiaretti-sim-results.RData")